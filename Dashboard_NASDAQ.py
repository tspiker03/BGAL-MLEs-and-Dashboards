...
def save_plots():
    ...
    for name, fig in st.session_state.figures:
        filepath = os.path.join('media', f'NDX_{selected_period}_{selected_frequency}_{name}.png')
        fig.savefig(filepath)
    
    # Save all plots as a single PDF
    pdf_path = os.path.join('media', f'NDX_{selected_period}_{selected_frequency}_report.pdf')
    with PdfPages(pdf_path) as pdf:
        # Add title page
        fig = plt.figure(figsize=(8.5, 11))
        fig.clf()
        title = f'NASDAQ-100 Analysis Report\nPeriod: {selected_period}\nFrequency: {selected_frequency}'
...
st.write("Index: NASDAQ-100 (^NDX)")
st.write("Volatility Index: NASDAQ-100 Volatility Index (^VXN)")

# Set stock symbol
stock_symbol = "^NDX"  # NASDAQ-100
...
if stock_symbol:
    ...
    if not data.empty and not vix_data.empty:
        ...
        for i in range(max_diff):
            ...
            st.subheader("ACF Plot of ^VXN")
            fig, ax = plt.subplots()
            sm.graphics.tsa.plot_acf(diff_data, lags=30, ax=ax)
            ax.set_title("Autocorrelation Function of ^VXN")
        ...
        if ar_model:
            ...
            st.subheader("Log Returns of NASDAQ-100 versus Time")
            st.line_chart(df["Log Returns"])

            st.subheader("ACF of Log Returns of NASDAQ-100")
            fig, ax = plt.subplots()
            ...
            st.subheader("Histogram of Log Returns of NASDAQ-100")
            fig, ax = plt.subplots()
...
Sigma_hat: {sigma_hat:.4f}"""
            ...
            st.subheader("Histogram of ^VXN~")
            fig, ax = plt.subplots()
            ax.hist(df["G~"].dropna(), density=True, bins=30)
            ax.set_title("Distribution of Simulated ^VXN")
            ax.set_xlabel("G~")
            ax.set_ylabel("Density")
            ...
            ax.set_title("Simulated BGAL vs Simulated ^VXN")
            ax.set_xlabel("G~")
        ...
        with col2:
            if st.button("Generate PDF Report"):
                save_plots()
                st.success(f"PDF report generated: NDX_{selected_period}_{selected_frequency}_report.pdf")
