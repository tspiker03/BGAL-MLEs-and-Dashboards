...
def save_plots():
    ...
    for name, fig in st.session_state.figures:
        filepath = os.path.join('media', f'NDX_Volume_{selected_period}_{selected_frequency}_{name}.png')
        fig.savefig(filepath)
    
    # Save all plots as a single PDF
    pdf_path = os.path.join('media', f'NDX_Volume_{selected_period}_{selected_frequency}_report.pdf')
    with PdfPages(pdf_path) as pdf:
        # Add title page
        fig = plt.figure(figsize=(8.5, 11))
        fig.clf()
        title = f'Nasdaq-100 Volume Analysis Report\nPeriod: {selected_period}\nFrequency: {selected_frequency}'
...
st.write("Using Nasdaq-100 Trading Volume")
...
if stock_symbol:
    ...
    if not data.empty:
        ...
        for i in range(max_diff):
            ...
            st.subheader("ACF Plot of Nasdaq-100 Log Volume")
            fig, ax = plt.subplots()
            sm.graphics.tsa.plot_acf(diff_data, lags=30, ax=ax)
            ax.set_title("Autocorrelation Function of Nasdaq-100 Log Volume")
...
Sigma_hat: {sigma_hat:.4f}"""
                ...
                with col2:
                    if st.button("Generate PDF Report"):
                        save_plots()
                        st.success(f"PDF report generated: NDX_Volume_{selected_period}_{selected_frequency}_report.pdf")
