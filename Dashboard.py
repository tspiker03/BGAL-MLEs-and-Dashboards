...
def save_plots():
    ...
    for name, fig in st.session_state.figures:
        filepath = os.path.join('media', f'SP500_{selected_period}_{selected_frequency}_{name}.png')
        fig.savefig(filepath)
    
    # Save all plots as a single PDF
    pdf_path = os.path.join('media', f'SP500_{selected_period}_{selected_frequency}_report.pdf')
    with PdfPages(pdf_path) as pdf:
        # Add title page
        fig = plt.figure(figsize=(8.5, 11))
        fig.clf()
        title = f'S&P 500 Analysis Report\nPeriod: {selected_period}\nFrequency: {selected_frequency}'
...
st.write("Volatility Index: CBOE Volatility Index (^VIX)")
...
if stock_symbol:
    ...
    if not data.empty and not vix_data.empty:
        ...
        max_diff = 3
        diff_data = np.log(df["Volatility"].dropna())
        ...
        ax.set_title("Autocorrelation Function of VIX")
...
Sigma_hat: {sigma_hat:.4f}"""
            ...
            ax.set_title("Distribution of Simulated VIX")
            ax.set_xlabel("G~")
            ax.set_ylabel("Density")
            ...
            ax.set_title("Simulated BGAL vs Simulated VIX")
            ax.set_xlabel("G~")
        ...
        with col2:
            if st.button("Generate PDF Report"):
                save_plots()
                st.success(f"PDF report generated: SP500_{selected_period}_{selected_frequency}_report.pdf")
