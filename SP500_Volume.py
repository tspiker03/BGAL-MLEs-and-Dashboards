...
def save_plots():
    ...
    for name, fig in st.session_state.figures:
        filepath = os.path.join('media', f'SP500_Volume_{selected_period}_{selected_frequency}_{name}.png')
        fig.savefig(filepath)
    
    # Save all plots as a single PDF
    pdf_path = os.path.join('media', f'SP500_Volume_{selected_period}_{selected_frequency}_report.pdf')
    with PdfPages(pdf_path) as pdf:
        # Add title page
        fig = plt.figure(figsize=(8.5, 11))
        fig.clf()
        title = f'S&P 500 Volume Analysis Report\nPeriod: {selected_period}\nFrequency: {selected_frequency}'
...
st.write("Index: S&P 500 (^GSPC)")
st.write("Using S&P 500 Trading Volume")

# Set stock symbol
stock_symbol = "^GSPC"  # S&P 500
...
if stock_symbol:
    ...
    if not data.empty:
        ...
        for i in range(max_diff):
            ...
            st.subheader("ACF Plot of S&P 500 Log Volume")
            fig, ax = plt.subplots()
            sm.graphics.tsa.plot_acf(diff_data, lags=30, ax=ax)
            ax.set_title("Autocorrelation Function of S&P 500 Log Volume")
        ...
        if ar_model:
            ...
            if len(g) > 0:
                ...
                df["Y~"] = delta_hat + mu_hat * df["G~"] + sigma_hat * np.sqrt(df["G~"])
...
Sigma_hat: {sigma_hat:.4f}"""
                ...
                ax.plot([min_val, max_val], [min_val, max_val], 'r--', label='line = s')
                ...
                with col2:
                    if st.button("Generate PDF Report"):
                        save_plots()
                        st.success(f"PDF report generated: SP500_Volume_{selected_period}_{selected_frequency}_report.pdf")
